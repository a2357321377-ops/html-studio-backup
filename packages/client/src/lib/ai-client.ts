...
8. **每页内容绝不能超出画幅** — 这是用户最常见的问题！每页内容必须在 960×540 可视区域内，可用内容区域约 768×396px。★★★ 防溢出硬规则 ★★★：(1) 绝不使用 position:absolute 或 position:fixed 定位内容（封面页的 brand/logo 小元素除外）；(2) 每个 slide 内所有内容必须自然流动在 flex/grid 布局中；(3) 标题字号不超过 h2(54px)，正文不超过 .lede(22px)；(4) 一页最多 1个标题 + 1个副标题/lede + 3-4个要点卡片，超过必须拆分到多页；(5) 每个 .card 内容不超过 3 行；(6) 避免文字过多，善用 .card、.grid、.kicker、.h1/.h2、.lede 等类
...
<script defer src="/html-ppt/assets/runtime.js"></script>
<script defer src="/html-ppt/assets/animations/fx-runtime.js"></script>
...
5. **内容绝不能超出画幅** — 这是最常被违反的规则！每页内容必须完全在 960×540 可视区域内。幻灯片 padding 为 72px 上下 + 96px 左右，可用内容区域约为 768×396px。每页最多放 3-4 个 .card，每个 .card 内容不超过 3 行。文字精炼，避免长段落。★★★ 防溢出硬规则 ★★★：(1) 绝不使用 position:absolute 或 position:fixed 定位内容（会脱离 overflow:hidden 的约束）；(2) 每个 slide 内所有内容必须自然流动在 flex/grid 布局中；(3) 标题字号不超过 h2(54px)，正文不超过 .lede(22px)；(4) 一页文字总量上限：1个标题 + 1个副标题/lede + 3-4个要点，超过此量必须拆分到多页
...
6. **保留模板的 data-fx 属性** — 模板中已有的 data-fx（如 cover 页的 constellation、thanks 页的 firework）必须保留。新增的 slide 如需 data-fx，参考模板已有 data-fx 的选择逻辑（封面用粒子类、结尾用烟花类、数据页用数据流类）
...
4. ★★★ 画幅检查 ★★★ — 规划每一页的内容量时，确保每页内容不超过可用区域 768×396px。内容过多时必须拆分到新页面，宁可多一页也不能让内容溢出画幅
5. 基于模板修改：替换 demo 内容为真实内容，按需增删 slide，保持模板视觉语言
6. 直接输出完整的 HTML 文档，从 <!DOCTYPE html> 开始，到 </html> 结束
7. 只输出 HTML 代码，不要输出任何解释文字、规划列表、页数说明等非 HTML 内容
8. 确保生成的 <section class="slide"> 数量恰好等于要求的页数`
    : `--- 生成流程（必须按此执行）---
1. 仔细阅读文档内容，提取所有关键信息点
2. 在脑中规划每一页的标题和布局类型，确保总数等于要求的页数（不要在输出中写出规划列表）
3. ★★★ 画幅检查 ★★★ — 规划每一页的内容量时，确保每页内容不超过可用区域 768×396px。内容过多时必须拆分到新页面，宁可多一页也不能让内容溢出画幅
4. 直接输出完整的 HTML 文档，从 <!DOCTYPE html> 开始，到 </html> 结束
5. 只输出 HTML 代码，不要输出任何解释文字、规划列表、页数说明等非 HTML 内容
6. 确保生成的 <section class="slide"> 数量恰好等于要求的页数`;
...
5. **内容绝不能超出画幅** — 每页内容在 960×540 可视区域内，可用内容区域约 768×396px。绝不使用 position:absolute/fixed 定位内容，所有内容自然流动在 flex/grid 布局中。一页最多 1个标题 + 1个副标题 + 3-4个要点卡片
