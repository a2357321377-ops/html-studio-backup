...
7. **只输出 HTML 代码** — 不要输出 markdown 代码围栏或任何解释文字
...
2. **每个 slide 必须加背景效果** — 绝不要留纯色白底！用内联 style 添加渐变、网格线、点阵等背景
3. **封面页和结尾页必须加 data-fx** — particle-burst、constellation、confetti-cannon、firework 让关键页面有视觉冲击力
4. **善用 .card 和 .grid 组合** — 不要把内容平铺，用 .card 包裹，用 .g2/.g3/.g4 分列
5. **善用 .kicker 和 .eyebrow** — 每页顶部加分类标签，让层次更清晰
6. **善用 .gradient-text** — 关键数字、标题关键词用渐变色突出
7. **善用 .counter** — 所有数字都应该用 counter 动画
8. **善用 .anim-stagger-list** — 列表和卡片组加交错动画
9. **善用 .anim-fade-up / .anim-rise-in** — 关键元素加进入动画
10. **不要使用 layout-* 类名**（如 layout-cover），它们不存在于 CSS 中
11. **不要使用 theme-* 类名**（如 theme-tokyo-night），主题通过 <link> 标签引入
12. **不要添加 <style>.slide { display: none; }</style>**，base.css 已处理幻灯片切换
13. **必须使用上述真实 CSS 类来排版**，否则幻灯片将没有任何样式
14. **表格必须用 <table> 标签**，配合 .card 使用，不要自己画表格
15. **图表用纯 CSS 实现**（柱状图用 flex + div，环形图用 conic-gradient），不要引入外部图表库
16. **视觉层次要分明**：kicker → h1/h2 → lede → dim，每页都有清晰的视觉层次
17. **data-fx 的值必须是上面列出的特效名之一**，不要自己编造名称
18. **不要连续两页使用相同布局**，保持视觉多样性
19. **每 3-4 页插入一个视觉冲击页**（stat-highlight、quote、section 分隔）
20. **内容不足时主动扩充** — 拆分长段落、添加 section 分隔、添加 quote/stat-highlight 页`;
...
2. 根据内容量规划页数，如果用户指定了页数则严格遵循
3. 先在脑中规划每一页的布局策略，确保布局多样、节奏感强
4. 然后逐页生成完整的 HTML 代码
5. 确保生成的 <section class="slide"> 数量等于用户要求的页数`;
